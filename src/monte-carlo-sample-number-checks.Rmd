...
find_optimal_B.phi = function (var.theta, theta, K, m, e, eps.mean, eps.sigma) {
  ...
  sigma2.phi = var((K-m)/(K-e-theta) * F_eps.m)
  dphi.dtheta = f_eps.m*F_eps.K - f_eps.K*F_eps.m - f_eps.K*mean((K-m)/(K-e-theta))
  
  var.theta^(-1) * sigma2.phi * dphi.dtheta^(-2)
}
```
...
find_optimal_B.psi = function (var.theta, theta, K, m, e, eps.mean, eps.sigma) {
  ...
  sigma2.psi = var((m-e-theta)/(K-e-theta) * F_eps.m)
  dpsi.dtheta =  mean((m-e-theta)/(K-e-theta)) * (f_eps.m*F_eps.m - f_eps.K/F_eps.K)
...
```{r}
F.m_theta = pnorm(m-init.CI$CI.lower, dating_error.mean, dating_error.sd)
F.K_theta = pnorm(K-init.CI$CI.lower, dating_error.mean, dating_error.sd)
