...
plot(B, var.theta_q$lower, type="b", col='blue', 
     ...
     ylim=c(min(c(var.theta_q$lower, var.theta_q$upper, var.theta.target)), max(c(var.theta_q$lower, var.theta_q$upper, var.theta.target))))
...
abline(h=var.theta_q$upper[which(abs(B - optimal_B.psi$upper)==min(abs(B - optimal_B.psi$upper)))], lty=1, col='grey')
abline(h=var.theta_q$lower[which(abs(B - optimal_B.psi$lower)==min(abs(B - optimal_B.psi$lower)))], lty=1, col='grey')
legend("bottomleft",
       c(paste0("var.theta.target=", var.theta.target), "q=0.025", paste0("optimal_B.lower=", optimal_B.psi$lower), "q=0.975", paste0("optimal_B.upper=", optimal_B.psi$upper)),
       lty=c(2, 1, 2, 1, 2),
       col=c('green', 'blue', 'blue', 'red', 'red'))
```
...
estimate_extinction.minmi(W, sd=fossil_dataframe$sd, K, speed=T)
```

```{r}
estimate_extinction.minmi(W, sd=fossil_dataframe$sd, K, level = 0.95, speed=T)
```

```{r}
estimate_extinction.minmi(W, sd=fossil_dataframe$sd, K, level = 0.95, B=500, speed=T)
```
