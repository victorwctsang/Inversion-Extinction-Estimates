...
estimate_var.delta_method = function (q, theta_q.hat, B, u, K, m, eps.mean, eps.sigma) {
  ...
  psi_hat = apply((m-e-theta_q.hat)/(K-e-theta_q.hat) * F_eps.m, 1, mean)
  psi_hat.prime = apply((m-K)/(K-e-theta_q.hat)^2 * F_eps.m, 1, mean)
  # 
  ...
  u_hat.prime = f_eps.K/F_eps.K^2 * psi_hat + psi_hat.prime/F_eps.K

  var_estimate = (1/F_eps.K)^2 * var(psi_hat) * (u_hat.prime)^(-2)  
...
for (i in 1:num.B) {
  ...
  for (j in 1:num.estimates) {
    ...
    exptd_var.upper[j] = estimate_var.delta_method(q=0.975, theta.hat.lower[j],
                                                   B[i], mc.samples, K, m,
                                                   dating_error.mean, dating_error.sd)
  }
