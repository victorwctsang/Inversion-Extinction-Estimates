...
for (i in 1:nThetas) {
  ...
  A1[i] = .calcA1(theta=theta.range[i], K=K, m=m[i], eps.mean=eps.mean, eps.sigma=eps.sigma)
  phi.hat[i] = .calcPhiHat(u=u, mean=eps.mean, sd=eps.sigma,
                           a=-Inf, b=m[i]-theta.range[i], K=K, m=m[i],
                           theta=theta.range[i])
  A2[i] = .calcA2(theta=theta.range[i], K=K, u=u, m=m[i], eps.mean=eps.mean, eps.sigma=eps.sigma)
...
res.df %>%
  ...
  geom_line() +
  labs(title=TeX("$P_\\theta (X + \\epsilon > m)$ for increasing theta"),
       x=TeX("\\theta"),
       y=TeX("P"))
```


## Check if components behave like probabilities
...
res.df %>%
  group_by() %>%
  summarise(min(A1), max(A1), min(A2), max(A2))
```
...
res.df %>%
  ...
  geom_line() +
  ggtitle("A1 for increasing Theta")
```
...
res.df %>%
  ...
  geom_line() +
  ggtitle("A2 for increasing Theta")
```
...
res.df %>%
  ...
  geom_line() +
  geom_smooth() +
  labs(title=TeX("\\hat{\\phi} for increasing \\theta"))
```
