...
m = P = A1 = A2 = phi.hat = equation1 = rep(NA, nThetas)
...
for (i in 1:nThetas) {
  W = simulateFossils(n=n,
                      theta=theta.range[i],
                      K=K,
                      eps.mean=eps.mean,
                      eps.sigma=eps.sigma)$W
  m[i] = min(W)
  A1[i] = .calcA1(theta=theta.range[i],
                  K=K,
                  m=m[i],
                  eps.mean=eps.mean,
                  eps.sigma=eps.sigma)
  phi.hat[i] = .calcPhiHat(u=u,
                           mean=eps.mean,
                           sd=eps.sigma,
                           a=-Inf,
                           b=m[i]-theta.range[i],
                           ...
                           m=m[i],
                           theta=theta.range[i])
  A2[i] = .calcA2(theta=theta.range[i],
                  K=K,
                  u=u,
                  m=m[i],
                  eps.mean=eps.mean,
                  eps.sigma=eps.sigma)
  P[i] = getP(theta=theta.range[i],
              K=K,
              u=u,
              m=m[i],
              eps.mean=eps.mean,
...
```{r}
res.df %>%
  mutate(above=m>theta) %>%
  ggplot(aes(theta, m, colour=above)) +
  geom_point() + 
  geom_abline(slope=1, intercept=0) +
  labs(title=TeX('Simulated m against true \\theta < K'),
       x=TeX("\\theta")) +
