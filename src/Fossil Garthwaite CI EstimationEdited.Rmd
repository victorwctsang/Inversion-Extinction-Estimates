...
```{r}
bound_lower = estimate_bound.rm(bound_lower, alpha/2, theta.hat.y, n, K, p, m, max_var=0.2*dating_error.sd, max_iter, eps.mean=0, eps.sigma=dating_error.sd)
bound_upper = estimate_bound.rm(bound_upper, 1-alpha/2, theta.hat.y, n, K, p, m, max_var=0.2*dating_error.sd, max_iter, eps.mean=0, eps.sigma=dating_error.sd)
...
```{r}
plot(na.omit(res$CI.iters[, "upper"]), pch=4, type="l", xlab="i", col='red', ylab=NA, main="CI Convergence",
     ylim=c(min(na.omit(res$CI.iters[, "lower"])), max(na.omit(res$CI.iters[, "upper"]))))

lines(na.omit(res$CI.iters[, "lower"]), pch=4, col='blue')
abline(h=theta.hat.y, col="black")
...
```{r}
res.optimal = estimate_CI.optimal_rm(W=fossil_dataframe$age,
                     K=18000,
                     alpha=0.05,
                     max_iter=2000,
                     eps.mean=0,
                     eps.sigma=mean(fossil_dataframe$sd),
                     return_iters=T,
                     theta_L=res$CI.lower,
                     theta_U=res$CI.upper,
                     B=500)
c(res.optimal$CI.lower, res.optimal$CI.upper)
res.optimal$CI.iters
```

```{r}
plot(na.omit(res.optimal$CI.iters[, "upper"]), pch=4, type="l", xlab="i", col='red', ylab=NA, main="CI Convergence",
     ylim=c(min(na.omit(res.optimal$CI.iters[, "lower"])), max(na.omit(res.optimal$CI.iters[, "upper"]))))

lines(na.omit(res.optimal$CI.iters[, "lower"]), pch=4, col='blue')
abline(h=theta.hat.y, col="black")
