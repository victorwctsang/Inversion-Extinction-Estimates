...
```{r}
all_widths.lineplot = all_widths %>%
  ggplot(aes(x=n, y=Width, colour=reorder(Method, desc(Width)))) +
  geom_line(size=0.5) +
  geom_point(size=2) +
  # geom_col(position="identity") +
  labs(y="Width (Years)", x=NULL, colour="Method")
all_widths.lineplot

ggsave(all_widths.lineplot, filename = "../figures/plot-application-widths-samplesize.svg")
```

```{r}
all_widths %>%
  merge(describe_tbl, by.x="species", by.y="dataset") %>%
  ggplot(aes(x=.range, y=Width, colour=Method)) +
  geom_line() +1
  geom_point() +
  ggtitle("Width against range")
```
