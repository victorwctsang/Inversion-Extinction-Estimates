...
estimate_extinction = function (dates, sd, K, theta.test_vec, alpha=0.05, label) {
  SIUGM = simulated_inversion(alpha=alpha, dates=dates, sd=sd, K=K, theta.test_vec = theta.test_vec)
  SIUGM$method = "SI-UGM"
  
  MINMI = estimate_extinction.minmi(W = dates, sd=sd, level=1-alpha, K=K)
  MINMI$method = "MINMI"
  
  SIMP = estimate_CI.rm(W = dates, K=K, alpha=alpha, max_iter=3000, eps.mean=0, eps.sigma=mean(sd))
  SIMP$method = "SI-RM"
  ...
  estimates = rbind(estimates, MINMI, SIUGM, SIMP)
