...
tmp = all_data %>%
  ...
  rbind(data.frame(age=mammoth$age, sd=mammoth$sd, dataset="Eurasian Mammoth")[sample(length(mammoth$age), 80), ]) %>%
  arrange(age) %>%
...
```{r}
ggsave(file="../figures/applications.svg", plot=p, height=5, width=10)
```
