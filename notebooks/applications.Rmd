...
describe_tbl = all_data %>%
  ...
  summarise(
    .min = min(age),
    .max = sort(unique(age), decreasing = TRUE)[2L],
    .K = max(age),
    .range = .max - .min,
    .sigma = mean(sd),
    .ratio = .sigma/.range
...
sigma.range.ratio %>%
  kable(booktabs=T, col.names = c("Dataset", "$\\sigma$", "$K$", "$\\sigma/K$"), format="latex", escape=F) %>%
...
all_widths_width.lineplot = all_widths %>%
  ...
  ggplot(aes(x=value, y=Width, colour=reorder(Method, desc(value)))) +
  geom_line(size=0.5) +
  geom_point() +
